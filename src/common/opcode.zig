...
/// Classifies every opcode by its operand layout, so the decoder knows which
...
pub const Opcode = enum(u8) {
    ...
    /// Return the decoder class for this opcode, used by `Instruction.fromBytes`
    /// to pick which byte fields to parse vs require-zero.
    ///
    /// Classification tables from Rust opcode.rs L32-175 (LOAD_IMM_OPS,
    /// LOAD_MEMORY_OPS, STORE_IMM_OPS, STORE_REG_OPS, BIN_IMM_OPS, BIN_REG_OPS,
    /// UNARY_OPS, JUMP_OPS, JUMP_IMM_OPS, JUMP_REG_OPS, CALL_IMM_OPS,
    /// CALL_REG_OPS, EXIT_OPS).
    pub fn operationType(self: Opcode) OperationType {
        return switch (self) {
            .Lddw => .LoadImmediate,

            .Ldxb, .Ldxh, .Ldxw, .Ldxdw => .LoadMemory,

            .Stb, .Sth, .Stw, .Stdw => .StoreImmediate,
