...
以下标准是 **C1 全部完成后** 才统一验证的验收门；当前实现过程中，
未勾选项不代表架构错误，只代表还未走到最终验收阶段。

C1 实现完成后，本架构的验收标准：

- [x] Zig 二进制不链接 libLLVM（`otool -L zig-out/bin/elf2sbpf` 仅见 `libSystem`）
- [x] 二进制体积 < 2MB（ReleaseSmall：~253 KB）
- [ ] 不出现跨层调用（例如 `common/` 不应 import `parse/`）
- [x] 9/9 zignocchio example 跟 shim 产物满足：
  - 3 个小 example 字节完全一致
  - 6 个大 example 也已达到字节完全一致
- [ ] 单次处理时间 < 100ms（最大 example；待正式 benchmark）
- [x] 无 memory leak（`zig build test` 绿，`std.testing.allocator` 未报 leak）
