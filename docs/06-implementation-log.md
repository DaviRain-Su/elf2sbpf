...
## D.7.10 V2.1 — Super-cluster (interleaved) rewriter ✅

**日期**：2026-04-18
**状态**：✅ 完成。pubkey 12.5× CU gap 完全闭合。

### 前置

V2.0 的 `hasInterleavedCluster` 守卫要求每个 cluster 独立 rewrite，
遇到 bpfel -O2 对 `std.mem.eql(u8, a, b)` 这种 32-byte compare 的
4-iteration 展开就只能放弃——pubkey 8 cluster 只能重写 2 个尾部
single，CU 从 187 只能降到 145（基线 15）。V2.1 做的是"多个互相
交织的 cluster 作为一个 super-cluster 一次性重写"。

### 做的事

**1. `src/ast/peephole.zig` +~380 行**
...
const RegTaint = struct {
    base_reg: u8,
    base_min: i16,
    mask: u64,  // bit i = offset (base_min + i) 是否参与
};
```

算法：
- `buildComponents`：union-find 把 pairwise interleaved cluster 合
  成连通分量。交织定义：任一 cluster 的 ldxb 节点落在另一 cluster
  的 `computeSpanEnd(span)` 范围内
- `taintPropagate`：在 super-span 内对每条指令做 SSA-like
  dataflow：
  - `Ldxb rX, [base+off]` → `taint[rX] = {base, off, mask = 0b1}`
  - `Lsh64Imm` → taint 不变（shift 不改 byte membership）
  - `Or64Reg rX, rY` → `taint[rX] = merge(taint[rX], taint[rY])`
  - 其它 whitelist 外的 opcode → `taint[dst] = null`
- `matchClustersToRegs`：每个 cluster 的目标 reg = taint.mask 正好
  `0xff`（8 个连续 bit）且 `base_reg/base_min` 对上的寄存器。歧义
  时（同 cluster 匹配多个 reg）拒绝整个 super-cluster
- 拓扑排序 ldxdw 发射顺序：如果 cluster A 的 dst_reg 是 cluster B
  的 base_reg，A 必须排在 B 之后（防止 self-clobber base）
- `applyRewriteSuper`：删除 super-span 全部节点，插入 N 条 ldxdw，
  renumber 后续 offset，fix numeric jumps/calls 跨越删除区的 off
- **Fallback**：super-cluster 任一安全检查失败 → 回退到逐个 member
  尝试 V2.0 单独重写路径。保证 V2.1 行为永远 ≥ V2.0

**2. 超参数 / 安全边界**

- `ext_budget = 2 * clusters.len + 2`：super_last 前向扩展上限。
  每个 cluster 需要最多 2 条 `lsh << 32; or` 做 low/high 合并
- 扩展时遇到 `Ldxb` 立即停——下一个 Ldxb 几乎肯定是下个 cluster
  或 scatter 字节 load，不是当前 super 的 trailing 合并
- Super-cluster size 上限 16（`member_clusters_buf` 静态大小）；
  溢出直接 skip

### 遇到的问题 + 解决

**Bug 1：ldxdw 发射顺序自写 base**

第一版按 ascending base_offset 发射。pubkey 尾部 super-cluster
`{0x28 (dst=r1), 0x48 (dst=r2)}`，两者 base_reg 都是 r1。按
0x28 先发射 → `ldxdw r1, [r1+0x28]` 覆盖了 r1 → 下条
`ldxdw r2, [r1+0x48]` 读到错误地址。

解决：拓扑排序"读 rX 的 ldxdw 必须在写 rX 的 ldxdw 之前"。

**Bug 2：贪心 forward extension 吞噬下一 cluster 的 ldxb**

V2.1 初版把 super_last 一直前向扩展到第一条非 whitelist 指令，
对 vault 等 compact 代码误把无关 ldxb 纳入 super-span → 整个
super-cluster 被 taint match 拒绝 → 回退前比 V2.0 损失 ~20 insns
节省。

解决：扩展时看到 Ldxb 立即停（下一个 Ldxb 肯定是新 cluster 或
scatter 字节 load），加上 `2*N+2` 步上限。

**Bug 3：V2.0 的 `hasInterleavedCluster` 被我替换掉后 vault 回归**

去掉 V2.0 的守卫后，vault 上本来 V2.0 能独立处理的 single cluster
现在落到 super-cluster 路径，taint match 失败时直接 skip 掉。

解决：加 fallback，super-cluster 失败时逐个 member 按 V2.0 单独
重写。

### 测试 / 结果

| 输入 | V2.0 尺寸 | V2.1 尺寸 | Delta |
|------|----------|----------|-------|
| pubkey.stock-zig.o | 1432 B | **424 B** | -1008 B / -126 insns |
| transfer.stock-zig.o | 552 B | 552 B | 持平（本来就是 single cluster）|
| vault.o | 11872 B | 11872 B | 持平（fallback 保住 V2.0 的 48 insn 节省）|

**pubkey V2.1 disassembly**（相对 V2.0 节省 126 insns）：
```
r5 = *(u64 *)(r1 + 0x10)     ; cluster 1.low
r6 = *(u64 *)(r1 + 0x30)     ; cluster 1.high
r2 = 0x1
*(u64 *)(r10 - 0x8) = r2
if r5 != r6 goto +0xc
r8 = *(u64 *)(r1 + 0x18)
...
r4 = *(u64 *)(r1 + 0x40)
if r8 != r4 goto +0x6
r2 = *(u64 *)(r1 + 0x48)     ; 0x48 先发 (读 r1)
r1 = *(u64 *)(r1 + 0x28)     ; 0x28 后发 (覆盖 r1)
r3 = 0x1
if r1 != r2 goto +0x1
r3 = 0x0
...
...
全 8 cluster 重写，等价于 solana-zig baseline 的 `ldxdw` codegen。
预期 CU 从 187 降到 ~15（基线）。

- 398/398 tests pass（+1 调整 pubkey 集成测试的预期 delta：
  - 从 V2.0 的 `default - 336` 改为 V2.1 的 `default - 1344`）
